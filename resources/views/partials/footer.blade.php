...
<footer class="relative bg-black text-white overflow-hidden" id="site-footer">
    ...
    <div class="relative border-b border-white/10" id="projects-we-support">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            {{-- Projects Grid (Grouped by Section) --}}
            @php
                // Separate projects: those without section first, then grouped by section
                $ungrouped = $supportedProjects->filter(fn($p) => empty($p->section_en));
                $grouped = $supportedProjects->filter(fn($p) => !empty($p->section_en))->groupBy('section_en');
            @endphp

            {{-- Ungrouped projects (no section) --}}
            @if($ungrouped->count() > 0)
            <div class="grid sm:grid-cols-2 lg:grid-cols-{{ min($ungrouped->count(), 4) }} gap-6 mb-8">
                @foreach($ungrouped as $project)
                    @include('partials._supported-project-card', ['project' => $project])
                @endforeach
            </div>
            @endif

            {{-- Grouped projects by section --}}
            @foreach($grouped as $sectionName => $sectionProjects)
                {{-- Section Divider --}}
                <div class="relative my-10 flex items-center justify-center">
                    <div class="absolute inset-0 flex items-center">
                        <div class="w-full border-t border-white/[0.08]"></div>
                    </div>
                    <div class="relative inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-black border border-accent-500/20">
                        @php
                            $sectionProject = $sectionProjects->first();
                            $sectionEs = $sectionProject->section_es ?? $sectionName;
                        @endphp
                        <span class="text-accent-400 text-xs font-bold tracking-[0.15em] uppercase"
                              x-text="$store.lang.current === 'en' ? '{{ addslashes($sectionName) }}' : '{{ addslashes($sectionEs) }}'"></span>
                    </div>
                </div>

                <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-4">
